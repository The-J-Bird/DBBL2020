...
// Print Player row
function print_prow($p, $x, $y, $h, $bgcolor='#FFFFFF', $bordercolor='#000000', $linewidth=1, $fontsize, $ma_color, $st_color, $ag_color, $pa_color, $av_color) {
  ...
  $this->Cell(17, $h, $p['nr'], 1, 0, 'C', true, '');
  $this->Cell(85, $h, $p['name'], 1, 0, 'L', true, '');
  $this->Cell(99, $h, $p['pos'], 1, 0, 'L', true, '');
  ...
  $this->Cell(18, $h, boolval($p['ag']) ? $p['ag']."+" : "", 1, 0, 'C', true, '');
  $this->SetFillColorBB($this->hex2cmyk($pa_color));
  //$this->Cell(18, $h, $p['pa']."+", 1, 0, 'C', true, '');
  $this->Cell(18, $h, ($p['pa']>0 && $p['pa']<7) ? $p['pa']."+" : (($p['st']==0) ? "" : "-"), 1, 0, 'C', true, '');
  $this->SetFillColorBB($this->hex2cmyk($av_color));
  //$this->Cell(18, $h, $p['av']."+", 1, 0, 'C', true, '');
  $this->Cell(18, $h, boolval($p['av']) ? $p['av']."+" : "", 1, 0, 'C', true, '');
  $this->SetFillColorBB($this->hex2cmyk($bgcolor));
  $this->SetXY(($x+17+85+99+18+18+18+18+18),$y);
  // Need to change to MultiCell to fit Skills and Injuries text if too long
  if ($newfontsize<$fontsize) {
    $this->SetFontSize($newfontsize);
    $this->MultiCell(279, $newheight, $p['skills'], 1, 'L', true);
    $this->SetFontSize($fontsize);
  }
  else 
    $this->MultiCell(279, $h, $p['skills'], 1, 'L', true);
  $this->SetXY($x+17+85+99+18+18+18+18+18+279,$y);
  $this->Cell(22, $h, $p['inj'], 1, 0, 'C', true, '');
  $this->Cell(19, $h, $p['cp'], 1, 0, 'C', true, '');
  $this->Cell(19, $h, $p['td'], 1, 0, 'C', true, '');
  $this->Cell(20, $h, $p['def'], 1, 0, 'C', true, '');
  $this->Cell(20, $h, $p['int'], 1, 0, 'C', true, '');
  $this->Cell(20, $h, $p['cas'], 1, 0, 'C', true, '');
  $this->Cell(21, $h, $p['mvp'], 1, 0, 'C', true, '');
  $this->Cell(21, $h, $p['misc'], 1, 0, 'C', true, '');
  $this->Cell(21, $h, $p['spp'], 1, 0, 'C', true, '');
  $this->Cell(38, $h, $p['value'], 1, 1, 'R', true, '');
...
function print_srow($p, $x, $y, $h, $bgcolor='#FFFFFF', $bordercolor='#000000', $linewidth=1, $fontsize) {
  ...
  // Needs to correct fontsize and height for skills, if text doesn't fit
  list($newheight, $newfontsize) = $this->FitTextInCell($h, 260, $fontsize, $p['skills']);
  if ($newheight<$h)
    $h=$newheight*2;
  $this->SetFontSize($fontsize);
  
  //Print cells in order
  //$this->Cell(170, $h, $p['name'], 0, 0, 'L', true, '');
  $this->Cell(170, $h, ($p['st']>0) ? $p['name'] : (($p['st']==0) ? "" : ""), 0, 0, 'L', true, '');
  $this->Cell(18, $h, $p['ma'], 0, 0, 'C', true, '');
  $this->Cell(18, $h, $p['st'], 0, 0, 'C', true, '');
  //$this->Cell(18, $h, $p['ag']."+", 0, 0, 'C', true, '');
  $this->Cell(18, $h, ($p['ag']>0) ? $p['ag']."+" : (($p['st']==0) ? "" : "-"), 0, 0, 'C', true, '');
  //$this->Cell(18, $h, $p['pa']."+", 0, 0, 'C', true, '');
  $this->Cell(18, $h, ($p['pa']>0) ? $p['pa']."+" : (($p['st']==0) ? "" : "-"), 0, 0, 'C', true, '');
  //$this->Cell(18, $h, $p['av']."+", 0, 0, 'C', true, '');
  $this->Cell(18, $h, ($p['av']>0) ? $p['av']."+" : (($p['st']==0) ? "" : "-"), 0, 0, 'C', true, '');
  $this->SetXY(($x+170+18+18+18+18+18),$y);
  // Need to change to MultiCell to fit Skills and Injuries text if too long
  if ($newfontsize<$fontsize) {
    $this->SetFontSize($newfontsize);
    $this->MultiCell(260, $newheight, $p['skills'], 0, 'L', true);
    $this->SetFontSize($fontsize);
  }
  else 
    $this->MultiCell(253, $h, $p['skills'], 0, 'L', true);
  $this->SetXY($x+170+18+18+18+18+18+260,$y);
if (array_key_exists('special',$p)) {
  $this->Cell(86, $h, $p['special'], 0, 0, 'L', true);}
  $this->Cell(19, $h, $p['cp'], 0, 0, 'C', true, '');
  $this->Cell(19, $h, $p['td'], 0, 0, 'C', true, '');
  $this->Cell(20, $h, $p['def'], 0, 0, 'C', true, '');
  $this->Cell(20, $h, $p['int'], 0, 0, 'C', true, '');
  $this->Cell(20, $h, $p['cas'], 0, 0, 'C', true, '');
  //$this->Cell(21, $h, $p['mvp'], 0, 0, 'C', true, '');
  $this->Cell(21, $h, $p['misc'], 0, 0, 'C', true, '');
...
function print_inducements($x, $y, $h, $bgcol, $linecol, $fontsize, $ind_name, $ind_amount, $ind_value) {
  ...
  $this->print_box(($currentx = $x), $y, 170, $h, $bgcol, $linecol, 0, 0, $fontsize, 'Helvetica', false, 'R', $ind_name);
  $this->print_box(($currentx += 170), $y, 15, $h, $bgcol, $linecol, 0, 0, $fontsize, 'Helvetica', false, 'C', $ind_amount);
  $this->print_box(($currentx += 15), $y, 15, $h, $bgcol, $linecol, 0, 0, $fontsize, 'Helvetica', false, 'C', $multiplier);
  $this->print_box(($currentx += 15), $y, 35, $h, $bgcol, $linecol, 0, 0, $fontsize, 'Helvetica', false, 'R', $ind_value);
}

function print_team_goods($x, $y, $h, $bgcol, $linecol, $perm_name, $perm_nr, $perm_value, $perm_total_value, $bold=false) {
  $this->print_box(($currentx = $x), $y, 40, $h, $bgcol, $linecol, 0, 0, 8, 'Helvetica', $bold, 'R', $perm_name);
  $this->print_box(($currentx += 40), $y, 20, $h, $bgcol, $linecol, 0, 0, 8, 'Helvetica', $bold, 'C', $perm_nr);
  $this->print_box(($currentx += 20), $y, 20, $h, $bgcol, $linecol, 0, 0, 8, 'Helvetica', $bold, 'C', 'x');
  $this->print_box(($currentx += 20), $y, 30, $h, $bgcol, $linecol, 0, 0, 8, 'Helvetica', $bold, 'R', $perm_value);
  $this->print_box(($currentx += 30), $y, 20, $h, $bgcol, $linecol, 0, 0, 8, 'Helvetica', $bold, 'C', '=');
  $this->print_box(($currentx += 20), $y, 45, $h, $bgcol, $linecol, 0, 0, 8, 'Helvetica', $bold, 'R', $perm_total_value);
}
