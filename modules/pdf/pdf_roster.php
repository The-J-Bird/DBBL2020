...
$pdf->AddFont('Helvetica','','helvetica.php');  // Adding regular font Tahoma which is in font dir
$pdf->AddFont('Helvetica','B','helveticab.php');  // Adding Tahoma Bold
...
$pdf->print_box($currentx, $currenty, 140, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', 'Total number of players next game:');
$pdf->print_box($currentx+=140, $currenty, 30, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', $sum_avail_players . '/' . $sum_players);

$pdf->SetX($currentx=MARGINX+6+538);
$pdf->print_box($currentx, $currenty, 50, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', 'Totals (excl TV for MNG players):');
$pdf->print_box($currentx+=54, $currenty, 19, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_cp);
$pdf->print_box($currentx+=19, $currenty, 19, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_td);
$pdf->print_box($currentx+=19, $currenty, 20, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_def);
$pdf->print_box($currentx+=20, $currenty, 20, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_int);
$pdf->print_box($currentx+=20, $currenty, 20, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_cas);
$pdf->print_box($currentx+=20, $currenty, 21, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_mvp);
$pdf->print_box($currentx+=21, $currenty, 21, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_misc);
$pdf->print_box($currentx+=21, $currenty, 21, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'C', $sum_spp);
$pdf->print_box($currentx+=21, $currenty, 38, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', $pdf->Mf($sum_pvalue));
...
if (($r == 'Nurgle') || ($r == 'Khemri') || ($r == 'Necromantic') || ($r == 'Undead')) {
  ...
  if (isset($ind_igor)) { 
    $ind_apo_igor_cost = $ind_igor*$inducements['Mortuary Assistant']['cost'];
...
$bribe_cost = $inducements['Bribes'][(($r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? 'reduced_cost' : 'cost')];
// Checking game data if cheaper biased referee for Goblin, Snotling, Black Orc and Underworld
$biasedref_cost = $inducements['Biased Referee'][(($r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? 'reduced_cost' : 'cost')];
// Checking game data if cheaper Biased Referee: Jorm the Ogre for Goblin, Snotling, Black Orc and Underworld
$bjorm_cost = $inducements['Biased Referee: Jorm the Ogre'][(($r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? 'reduced_cost' : 'cost')];
// Checking game data if cheaper Trundlefoot Triplets for Halflings
$btrundle_cost = $inducements['Biased Referee: The Trundlefoot Triplets: Bungo, Filibert and Jeph'][(($r == 'Halfling') ? 'reduced_cost' : 'cost')];
...
if (isset($ind_rr)) { $ind_cost += $ind_rr*$inducements['Extra Team Training']['cost']; $ind_count += 1; }
...
if ($ind_count > 0 ) {
  $pdf->print_box($currentx, $currenty, 170, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', 'Inducements ');
  $pdf->print_box(($currentx += 170), $currenty, 120, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'L', '(for next match)');
}
  $pdf->print_box(($currentx = 630), $currenty, 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', 'Team Goods'); // 156 to margin
...
$ind_display_counter = 0;
  ...
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;} 
  if (isset($ind_rr)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Extra Team Training (0-8):', $ind_rr, $pdf->Mf($inducements['Extra Team Training']['cost']));}
...
//dedicated fans
$pdf->print_box($currentx+=70, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', 'Dedicated Fans:' );
$pdf->print_box($currentx+=40, ($currenty), 65, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', $pdf->Mf($team->rg_ff));
//treasury
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', 'Treasury:' );
$pdf->print_box($currentx+=40, ($currenty), 65, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', false, 'R', $pdf->Mf($team->treasury));

// Team Value, Inducements Value, Match Value
$h=13;
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', 'Team Value (incl MNGs value):');
$pdf->print_box($currentx+=40, ($currenty), 65, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', $pdf->Mf($team->value+$sum_p_missing_value));
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', 'Induced Value:');
$pdf->print_box($currentx+=40, ($currenty), 65, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', $pdf->Mf($ind_cost));
$pdf->print_box($currentx-=40, ($currenty+=$h), 40, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', 'Current Team Value:');
$pdf->print_box($currentx+=40, ($currenty), 65, $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 0, 0, 8, 'Helvetica', true, 'R', $pdf->Mf($team->value + $ind_cost));
