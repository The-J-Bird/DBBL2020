...
#        $pdf->RoundedRect($currentx + 475, $currenty + $scoreboxOffset, 50, 50, 5, 'D');
	...
	// Gate + score text
        ...
        $currenty += 10;
        $pdf->SetXY($currentx,$currenty);
        $pdf->Cell(210, 50, "Fan Attendance:${space}k", 0, 0, 'R', false, '');
       ...
       //Printing headers for player rows. Do all this twice
        ...
        while ($i < 2) {
            ...
            $pdf->Cell(60, $h, 'Dedicated Fans', 1, 0, 'C', true, '');
            $pdf->Cell(60, $h, 'Total team CAS', 1, 0, 'C', true, '');
            $pdf->Cell(50, $h, 'Fan Factor', 1, 0, 'C', true, '');
            ...
            $pdf->Cell(60, $h, '', 1, 0, 'C', true, '');
            $pdf->Cell(50, $h, '', 1, 0, 'C', true, '');


          if (1 && $FILLED) {
                    $t = ${"team$i"};
                    $pdf->SetFont('Helvetica','',7);
                    $statsstr = sprintf('CTV: %uk - ReRolls: %u - Apothecary: %u - Dedicated Fans: %u - Assistant Coaches: %u - Cheerleaders: %u - Played: %u - Win pct.: %1.0f - ELO: %1.0f - CAS inflicted: %u', $t->tv/1000, $t->rerolls,$t->apothecary,$t->rg_ff,$t->ass_coaches,$t->cheerleaders,$t->mv_played,$t->rg_win_pct, $t->rg_elo, $t->mv_cas);
            ...
            $pdf->Cell($smallFieldSize, $h, 'Def', 1, 0, 'C', true, '');
