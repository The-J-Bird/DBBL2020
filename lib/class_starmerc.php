...
public $cost = 0;
    ...
    public function getStats($type, $type_id) {
        global $STATS_TRANS;
        $fields = array('cp','td','deflct','intcpt','mvp','bh+ki+si', 'bh','si','ki','misc');
	...
	public function setSpecialRules($makeString = false) {
        $query = "SELECT special FROM game_data_stars WHERE star_id = $this->star_id";
        $result = mysql_query($query);
        list($specialstr) = mysql_fetch_row($result);
        $this->special = ($makeString) ? specialsTrans($specialstr) : (empty($specialstr) ? array() : explode(',', $specialstr));
    }
