...
$key = 	substr_replace($key ,"", -1);
	...
	***************************/
	public function TranslateSpecialRules() {		
	    global $specialruleididx;
	    foreach($specialruleididx as &$specialrule) {
		    $specialrule = $this->getTrn('specialrules/'.strtolower(str_replace(array(' ','-','&',"'",'/'),'',$specialrule)));
	    }
	    unset($specialrule);
	}
}