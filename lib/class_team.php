...
public $retired = 0;
    ...
    public function isMaxBigGuys() {
        /**
         * Returns true/false depending on, if the team may purchase more big guys/has reached the max. big guy limit.
         **/
        global $rules;
        setupGlobalVars(T_SETUP_GLOBAL_VARS__LOAD_LEAGUE_SETTINGS, array('lid' => $this->f_lid)); // Load correct $rules for league.
        $query = "SELECT (COUNT(*) >= (SELECT r.bigguy_qty FROM races r JOIN teams t on r.race_id = t.f_race_id 
			WHERE t.team_id = $this->team_id))  FROM players p
			INNER JOIN game_data_players gdp ON gdp.pos_id = p.f_pos_id
            WHERE p.owned_by_team_id = $this->team_id AND p.date_sold IS NULL AND gdp.is_bigguy = 1 AND p.status NOT IN (".DEAD.")";
        $result = mysql_query($query);
        $row = mysql_fetch_row($result);
        return (bool) $row[0];
    }
